...
func LerpEffect(src image.Image, xRatio, yRatio float64, x, y int) color.RGBA64 {
	...
	// alpha beta 近傍４画素の座標の3つでleaper初期化
	l := lerp.NewLerper(alpha, beta, c)

	// RGBAそれぞれの値に対してLerpを適用
	r := l.Lerp(initGetOneColorFunc(src, "R"))
	g := l.Lerp(initGetOneColorFunc(src, "G"))
	b := l.Lerp(initGetOneColorFunc(src, "B"))
	a := l.Lerp(initGetOneColorFunc(src, "A"))
